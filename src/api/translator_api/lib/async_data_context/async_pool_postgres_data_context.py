...
class AsyncPoolPostgresDataContext(AsyncDataContext):

    @property
    def conn_string(self) -> str:
        return f'postgres://{self._user}:{self._password}@{self._host}:{self._port}/{self._database}'

    def __init__(self, host: str, port: int, database: str, user: str, password: str, loop: AbstractEventLoop) -> None:
        self._host = host
        self._port = port
        self._database = database
        self._user = user
        self._password = password
        self._pool: Pool = loop.run_until_complete(asyncpg.create_pool(dsn=self.conn_string, command_timeout=60))
